...
static uint16_t processReceivedPackets ( )
{
...
static uint16_t processCyclic ( )
{
  ...
  do {
    ...
    uint8_t * txData = ST_STREAM_PAYLOAD( txEnd );
    toTx = 0;
    status = applProcessCyclic( &protocol, &toTx, txData, ST_STREAM_MAX_DATA_SIZE - txSize );
...
void StreamDispatcherInitAndConnect ()
{
  StreamDispatcherInit();
  StreamConnect();
}

void StreamDispatcherInit ()
{
  StreamDispatcherGetLastError();
  StreamInitialize( rxBuffer, txBuffer );
}

uint8_t StreamDispatcherGetLastError( )
{
